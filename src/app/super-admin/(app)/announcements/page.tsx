...
export default function SuperAdminAnnouncementsPage() {
  const { announcements, addAnnouncement, toggleAnnouncement, addToast } = useSuperAdminStore();

  const [modalOpen, setModalOpen] = useState(false);
  const [title, setTitle] = useState("");
  const [message, setMessage] = useState("");
  const [severity, setSeverity] = useState<"info" | "warning" | "critical">("warning");
  const [targetTenants, setTargetTenants] = useState<"all" | "enterprise_only" | "trial_only">("all");
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-black text-[#0F172A] tracking-tight">System Announcements Broadcast</h1>
            ...
            <p className="text-xs text-[#64748B] mt-0.5">
              Broadcast maintenance schedules, new AI feature releases, and emergency carrier notices to client tenant dashboards.
            </p>
          </div>
        </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-[10px] text-[#64748B]">Target: {anc.targetTenants.replace("_", " ")}</span>
                  </div>
            ...
            <form onSubmit={handleCreateAnnouncement} className="space-y-3.5 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    value={targetTenants}
                    onChange={(e: any) => setTargetTenants(e.target.value)}
                    className="w-full px-3.5 py-2.5 bg-white border border-[#E2E8F0] rounded-xl outline-none focus:border-[#3157D5] text-[#0F172A]"
                  >
                    <option value="all">All Tenant Orgs</option>
                    <option value="enterprise_only">Enterprise Only</option>
                    <option value="trial_only">Trial Accounts Only</option>
                  </select>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Full announcement text to display on tenant dashboards..."
                  className="w-full px-3.5 py-2.5 bg-white border border-[#E2E8F0] rounded-xl outline-none focus:border-[#3157D5] text-[#0F172A]"
                />
              </div>
