...
export default function ConversationFunnelsPage() {
  const { funnelSteps, calls } = useAppStore();
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-5 bg-white rounded-3xl border border-[#E5EAF2] card-shadow">
          ...
          <div className="text-2xl font-bold text-[#172033] mt-1">{calls.length > 0 ? "10,000 Callers" : "0 Callers"}</div>
          <span className="text-xs text-[#16A36A] font-semibold mt-1 block">{calls.length > 0 ? "100% Dialog Initiated" : "0% Dialog Initiated"}</span>
        </div>

        <div className="p-5 bg-white rounded-3xl border border-[#E5EAF2] card-shadow">
          <span className="text-xs font-semibold text-[#78849A] uppercase tracking-wider">End-to-End Retention</span>
          <div className="text-2xl font-bold text-[#16A36A] mt-1">{calls.length > 0 ? "58.4% Finalized" : "0.0% Finalized"}</div>
          <span className="text-xs text-[#78849A] mt-1 block">{calls.length > 0 ? "5,840 booked meetings & confirmed SMS" : "0 booked meetings & confirmed SMS"}</span>
        </div>

        <div className="p-5 bg-white rounded-3xl border border-[#E5EAF2] card-shadow">
          <span className="text-xs font-semibold text-[#78849A] uppercase tracking-wider">Highest Friction Turn</span>
          <div className="text-2xl font-bold text-[#D95C68] mt-1">{calls.length > 0 ? "Step 3 (-12.9%)" : "None (-0.0%)"}</div>
          <span className="text-xs text-[#78849A] font-semibold mt-1 block">{calls.length > 0 ? "Qualification / Volume Hesitation" : "No friction detected"}</span>
        </div>
      </div>
